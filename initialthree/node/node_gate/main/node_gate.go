...
func main() {
	...
	ret := hutils.Must(config.GetConfig(conf, os.Args[2], os.Args[3])).(*config.Gate)
	_addr := hutils.Must(ret.MakeAddr()).(addr.Addr)

	filename := fmt.Sprintf("node_gate_%s.log", _addr.Logic.String())
	...
	zaplogger.GetSugar().Debugf("log %v", conf.Log)

	db.FlyfishInit(conf.Common.DbConfig, zaplogger.GetLogger())

	config.MustStartCluster(conf, _addr, serverType.Gate, uniLocker.New(db.GetFlyfishClient("nodelock")))

	node_gate.ReportInit(ret.ExternalAddr)
	node_gate.InitGroups(conf.Common.ServerGroups)

	if address, ok := otherStart.Has(otherStart.PROMETHEUS); ok {
		monitor.StartPrometheus(address)
	}

	hutils.Must(nil, node_gate.Start(ret.ExternalAddr))
