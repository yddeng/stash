...
const (
	dbDataTableName  = "global_data"
	dbDataTableKey   = "firewall"
	dbDataTableField = "data"
)
...
func AuthUserID(fc *flyfish.Client, userID string, cb func(exist bool)) {
	...
	GetCmd(fc, userID).AsyncExec(func(r *flyfish.GetResult) {
		if r.ErrCode == nil {
			fc.IncrBy(dbTable, userID, dbFieldAccessTimes, 1).AsyncExec(func(*flyfish.ValueResult) {})
			cb(true)
