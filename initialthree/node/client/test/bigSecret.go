...
func main() {
	...
	login.Login(userID, addr, func(lSession *login.Session, d *dispatcher.Dispatcher, msg *codecs.Message, err error) {
		if nil != err {
			...
			d.Register(cmdEnum.CS_BigSecretSync, func(session *fnet.Socket, msg *codecs.Message) {
				data := msg.GetData().(*cs_msg.BigSecretSyncToC)
				fmt.Println("------ BigSecretSyncToC", data)
			})
			d.Register(cmdEnum.CS_BigSecretCompetitionSync, func(session *fnet.Socket, msg *codecs.Message) {
				data := msg.GetData().(*cs_msg.BigSecretCompetitionSyncToC)
				fmt.Println("------ BigSecretCompetitionSyncToC", data)
			})
