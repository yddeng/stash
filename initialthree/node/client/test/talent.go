...
func main() {
	...
	login.Login(userID, addr, func(lSession *login.Session, d *dispatcher.Dispatcher, msg *codecs.Message, err error) {
		if nil != err {
			...
			d.Register(cmdEnum.CS_TalentSync, func(session *fnet.Socket, msg *codecs.Message) {
				data := msg.GetData().(*cs_msg.TalentSyncToC)
				fmt.Println("TalentSyncToC", data)
			})
			d.Register(cmdEnum.CS_TalentLevelUp, func(session *fnet.Socket, msg *codecs.Message) {
				data := msg.GetData().(*cs_msg.TalentLevelUpToC)
				fmt.Println("TalentLevelUpToC", data)
				cmd(lSession)
			})
			d.Register(cmdEnum.CS_TalentReset, func(session *fnet.Socket, msg *codecs.Message) {
				data := msg.GetData().(*cs_msg.TalentResetToC)
				fmt.Println("TalentResetToC", data)
				cmd(lSession)
			})
