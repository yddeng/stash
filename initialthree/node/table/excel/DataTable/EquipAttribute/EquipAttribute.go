...
func GetID(key int32)*EquipAttribute{
	v := Table_.indexID.Load().(map[int32]*EquipAttribute)[key]
	if v == nil{
...
func GetIDMap() map[int32]*EquipAttribute {
	return Table_.indexID.Load().(map[int32]*EquipAttribute)
}
...
func init(){
	Table_.xlsxName = "EquipAttribute.xlsx"
...
func (this *Table)Load(path string){
	...
	tmpIDMap := map[int32]*EquipAttribute{}
	...
	for _,row := range rows{
		if row[0] != "" {
            ...
            e.AttributeDesc = excel.ReadStr(readCellValue(row, names, "AttributeDesc"))
