...
import (
	...
	"initialthree/node/node_game/module/talent"
...
type transactionTalentReset struct {
...
func (t *transactionTalentReset) Begin() {
	defer func() { t.EndTrans(&cs_message.SignInToC{}, t.errcode) }()

	t.errcode = cs_message.ErrCode_OK
	zaplogger.GetSugar().Infof("%s TalentReset ", t.user.GetUserLogName())

	talentModule := t.user.GetSubModule(module.Talent).(*talent.UserTalent)
...
func (t *transactionTalentReset) GetModuleName() string {
	return "Talent"
}

func init() {
	user.RegisterTransFunc(cmdEnum.CS_TalentReset, func(user *user.User, msg *codecs.Message) transaction.Transaction {
		return &transactionTalentReset{
			user: user,
			req:  msg,
		}
	})
