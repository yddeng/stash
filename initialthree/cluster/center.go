...
func (this *Cluster) centerInit(export ...bool) {
	...
	this.centerClient.RegisterCenterMsgHandler(center_proto.CENTER_HeartbeatToNode, func(session *fnet.Socket, msg proto.Message) {
		//心跳响应暂时不处理
	...
	this.centerClient.RegisterCenterMsgHandler(center_proto.CENTER_NotifyNodeAdd, func(session *fnet.Socket, msg proto.Message) {
		NodeAdd := msg.(*center_proto.NodeAdd)

		logger.Sugar().Infof("%s CENTER_NotifyNodeAdd %v\n", this.serverState.selfAddr.Logic.String(), nodes2Str(NodeAdd.Nodes))
	...
	this.centerClient.RegisterCenterMsgHandler(center_proto.CENTER_NotifyNodeInfo, func(session *fnet.Socket, msg proto.Message) {

		currentEndPoints := this.serviceMgr.getAllNodeInfo()

		NotifyNodeInfo := msg.(*center_proto.NotifyNodeInfo)

		logger.Sugar().Infof("%s CENTER_NotifyNodeInfo %v\n", this.serverState.selfAddr.Logic.String(), nodes2Str(NotifyNodeInfo.Nodes))
	...
	this.centerClient.RegisterCenterMsgHandler(center_proto.CENTER_NodeLeave, func(session *fnet.Socket, msg proto.Message) {
