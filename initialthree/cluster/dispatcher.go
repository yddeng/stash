...
func (this *msgManager) register(cmd uint16, handler MsgHandler) {
	...
	if nil == handler {
		//记录日志
		logger.Sugar().Errorf("Register %d failed: handler is nil\n", cmd)
		return
	}
	_, ok := this.msgHandlers[cmd]
	if ok {
		//记录日志
		logger.Sugar().Errorf("Register %d failed: duplicate handler\n", cmd)
		return
	}
...
func (this *msgManager) dispatch(from addr.LogicAddr, cmd uint16, msg proto.Message) {
	...
	if ok {
		...
		//记录日志
		logger.Sugar().Errorf("unkonw cmd:%d\n", cmd)
	}
}
	...
	if nil != h {
		defer func() {
			if r := recover(); r != nil {
				...
				logger.Sugar().Errorf("error on onPeerDisconnected\nstack:%v,%s\n", r, buf[:l])
			}
		}()
		h(peer, err)
	}
...
func pcall(handler MsgHandler, from addr.LogicAddr, cmd uint16, msg proto.Message) {
	defer func() {
		if r := recover(); r != nil {
			buf := make([]byte, 65535)
			l := runtime.Stack(buf, false)
			logger.Sugar().Errorf("error on Dispatch:%d\nstack:%v,%s\n", cmd, r, buf[:l])
		}
	}()
	handler(from, msg)
}

func (this *Cluster) dispatch(from addr.LogicAddr, session *fnet.Socket, cmd uint16, msg proto.Message) { //from addr.LogicAddr, cmd uint16, msg proto.Message) {
	if this.IsStoped() {
		return
	}
	...
	if nil != msg {
		...
		case *cluster_proto.Heartbeat:
			...
			break
		case *cluster_proto.LbReport:
			req := msg.(*cluster_proto.LbReport)
			setLB(addr.LogicAddr(req.GetLogic()), int(req.GetPriority()), int(req.GetCapacity()))
		default:
