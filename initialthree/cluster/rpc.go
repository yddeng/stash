 ...
 */
func (this *Cluster) asynCall(peer addr.LogicAddr, end *endPoint, arg proto.Message, timeout time.Duration, callback rpc.RPCResponseHandler, donthookCallback ...bool) {
	end.Lock()
	defer end.Unlock()
	...
	if nil != end.session {
		if err := this.rpcMgr.asynCall(&RPCChannel{
			...
			peer:    end,
			cluster: this,
		}, "call", arg, timeout, cb); nil != err {
			//记录日志
			logger.Sugar().Errorf("Call %s:%s error:%s\n", end.addr.Logic.String(), reflect.TypeOf(arg).String(), err.Error())
			cb(nil, err)
		} else {
		...
		c.listElement = end.pendingCall.PushBack(c)
		c.dialTimer = timer.Once(timeout, func(t *timer.Timer, ctx interface{}) {
			end.Lock()
			if nil != c.listElement {
				end.pendingCall.Remove(c.listElement)
				c.listElement = nil
			}
			end.Unlock()
...
*  异步RPC调用
 */
func (this *Cluster) AsynCall(peer addr.LogicAddr, arg proto.Message, timeout time.Duration, callback rpc.RPCResponseHandler) {
		...
		if peer.Empty() {
			...
			if nil == endPoint {
				if peer.Group() == this.serverState.selfAddr.Logic.Group() {
					//记录日志
					logger.Sugar().Errorf("Call %s not found", peer.String())
