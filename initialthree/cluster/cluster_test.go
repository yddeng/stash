...
func init() {
	logger := zaplogger.NewZapLogger("test_cluster.log", "log", "debug", 100, 14, 10, true)
	InitLogger(logger)
...
func TestBothConnect(t *testing.T) {
	...
	go func() {
		...
		node1.AsynCall(nodeAddr, &ss_rpc.EchoReq{Message: proto.String("hello")}, time.Second, func(result interface{}, err error) {
			fmt.Println(err)
			if nil != result {
				close(cc1)
			}
	...
	go func() {
		...
		node2.AsynCall(nodeAddr, &ss_rpc.EchoReq{Message: proto.String("hello")}, time.Second, func(result interface{}, err error) {
			fmt.Println(err)
			if nil != result {
				close(cc2)
			}
...
func TestExportMethod(t *testing.T) {
	...
	node1.RegisterMethod(&ss_rpc.EchoReq{}, func(replyer *rpc.RPCReplyer, arg interface{}) {
		...
		logger.Sugar().Infof("echo message:%s\n", echo.GetMessage())
		echoResp := &ss_rpc.EchoResp{}
		echoResp.Message = proto.String("world")
		replyer.Reply(echoResp, nil)
	})
	...
	AsynCall(node1Addr, &ss_rpc.EchoReq{Message: proto.String("hello")}, time.Second, func(result interface{}, err error) {
	})
	...
	AsynCall(addr.LogicAddr(0), &ss_rpc.EchoReq{Message: proto.String("hello")}, time.Second, func(result interface{}, err error) {
		cc <- struct{}{}
	})
...

	{
		...
		AsynCall(nodeAddr, &ss_rpc.EchoReq{Message: proto.String("hello")}, time.Second, func(result interface{}, err error) {
			cc <- struct{}{}
		})
...
func TestRPC(t *testing.T) {
	...
	//异步调用
	node2.AsynCall(node1Addr, &ss_rpc.EchoReq{Message: proto.String("hello")}, time.Second, func(result interface{}, err error) {
		assert.Equal(t, result.(*ss_rpc.EchoResp).GetMessage(), "world")
		cc <- struct{}{}
	})
...
func TestHarbor(t *testing.T) {
	...
	node2.AsynCall(peer, &ss_rpc.EchoReq{Message: proto.String("hello")}, time.Second, func(result interface{}, err error) {
		assert.Equal(t, result.(*ss_rpc.EchoResp).GetMessage(), "world")
		cc <- struct{}{}
	})
